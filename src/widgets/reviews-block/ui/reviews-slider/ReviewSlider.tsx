...
export const ReviewSlider: React.FC<IReviewSliderProps> = memo((props) => {
   const { isMobile, limit = 4, sortValue } = props
   ...
   const onGetValueHandle = useCallback(
      (index: number) => {
      ...
      return (
         <div
            className={classNames(
               isMobile ? style.wrapper_mobile : style.wrapper
            )}>
      ...
      <div
         ...
         {!isError && (
            <SliderManagment
               ...
               onClickNext={onClickNextHandle}
               onClickPrev={onClickPrevHandle}
               maxValue={
                  (data?.totalCount || 0) < limit ? data?.totalCount : limit
               }
               isShowViewValue
            />
         )}
