...
import { ILikesScheme } from "../types";
...
const likesSlice = createSlice({
    ...
    reducers: {
        toggleLikeExcursion: (state, action: PayloadAction<string>) => {
            let like = state.excursionIds.find(item => item === action.payload);

            if(like)
                state.excursionIds = state.excursionIds.filter(item => item !== like);
            else
                state.excursionIds.push(action.payload)
        },
        saveLikes: (state) => {
