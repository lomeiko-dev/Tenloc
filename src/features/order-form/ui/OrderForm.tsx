...
export const OrderForm: React.FC<IOrderFormProps> = memo((props) => {
    ...
    const openModalHandle = useCallback(() => {
        if(valueAdult === 0 && valueChildren === 0 && 
           valuePensioner === 0 && valueYouth === 0 && valueSmallChildren === 0 ){
        ...
        valueAdult, valuePensioner, valueChildren, valueYouth, valueSmallChildren])

    useEffect(() => {
        setResultPrice((valueAdult * (priceAdult || 0)) +
    ...
    return(
        <div 
            ...
            <Field 
                isReadOnly
                padding="0 20px" margin="30px 0 0 0" 
            ...
            <Field 
                ...
                selection={timeFrame} value="Выбрать время"/>

            <div className={style.items}>
                {priceAdult !== undefined &&
                    <PriceItem onChangeValue={setValueAdult} name="Взрослый" price={priceAdult}/>}
                
                {pricePensioner !== undefined &&
                    <PriceItem name="Пенсионер" onChangeValue={setValuePensioner} price={pricePensioner}/>}
                
                {priceYouth !== undefined &&
                    <PriceItem name="Дети с 15 до 18" price={priceYouth} onChangeValue={setValueYouth}/>}
                
                {priceChildren !== undefined &&
                    <PriceItem name="Дети до 14 лет" onChangeValue={setValueChildren} price={priceChildren}/>}
                
                {priceSmallChildren !== undefined &&
                    <PriceItem name="Дети до 7 лет" onChangeValue={setValueSmallChildren} price={priceSmallChildren}/>}
            </div>
            <div className={style.result}>
                <Text fontSize={14} fontWeight={500} text="Итого"/>
                <Text color="#454545" fontSize={24} fontWeight={600} text={`${String(resultPrice)} ₽`}/>
            </div>
            <Button 
                margin="29px 0 0 0"
            ...
            <Modal
                lazy width="1075px" height="95%" maxHeight="1000px"
                ...
                <RegistrationOderFormLazy
                    ...
                    valueAdult={valueAdult} valueChildren={valueChildren}
                    valuePensioner={valuePensioner} valueSmallChildren={valueSmallChildren}
