...
const ExcursionPage = () => {
   ...
   const [isMiddleMobile, setMiddleMobile] = useState<boolean | undefined>(
      undefined
   )
   ...
   if (isError) navigate(pathRoutes.internal_server_error.path)
      ...
      <Page
         ...
         <div className={style.head}>
            <Text
               ...
               styleText={
                  isMiddleMobile
                     ? enumStyleText.QUATERNARY_SUBTITLE
                     : enumStyleText.QUATERNARY_TITLE
               }
               text={data[0].name}
            />
            <div className={style.price}>
               <Text
                  className={style.price_value}
                  color="#252525"
                  fontSize={isMiddleMobile ? 14 : 29}
                  fontWeight={700}
                  text={`от ${data[0].priceMiddle} ₽`}
               />
               <Text
                  className={style.price_title}
                  color="#828282"
                  fontSize={isMiddleMobile ? 13 : 15}
                  fontWeight={400}
                  text="за человека"
               />
            </div>
         ...
         <Carousel
            margin="49px 0 0 0"
            positionPx={isMobile ? 0 : -150}
            valueSkipPx={valueSkipPx}
            classNameContent={style.carousel_content}
         ...
         <div className={style.content}>
            <div className={style.left_part}>
               ...
               <PrimaryInfo
                  margin="45px 0 0 0"
                  time={data[0].time}
                  typeExcursion={data[0].typeExcursion}
                  isFreeCancellation={data[0].isFreeCancellation}
                  sizeGroup={data[0].sizeGroup}
               />
               <DetailedDescription
                  margin="42px 0 0 0"
                  width="952px"
            ...
            <div className={style.right_part}>
               <OrderForm
                  errorAuth={() => setShowModalLogin(true)}
         ...
         {showModalLogin && (
            <AuthModal onCloseModal={() => setShowModalLogin(false)} />
         )}
      </Page>
   )
}
