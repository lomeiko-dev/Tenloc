...
export const ArticleCard: React.FC<IArticleCardProps> = memo((props) => {
  ...
  const getDate = useCallback(() => {
    ...
    if (word === 'Сегодня' || word === 'Вчера') { return `${word} ${timeCreate}` }
  ...
  return (
        <div
            ...
            <div className={style.date_block}>
                ...
                <Text
                    color="#828282" fontSize={13} fontWeight={400}
                    text={getDate()}/>
            </div>
            <Text
                margin={isMobile ? '10px 0 0 0' : '16px 0 0 0'}
                height={isMobile ? '70px' : '54px'} width={isMobile ? '251px' : '324px'}
                fontSize={16} fontWeight={500} lineHeight={25}
                text={title}/>
            <Button
                padding="7px"
                onClick={onClickLink}
